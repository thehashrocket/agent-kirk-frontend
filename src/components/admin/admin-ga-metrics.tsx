/**
 * @file src/components/admin/admin-ga-metrics.tsx
 * Admin version of GA metrics component that displays analytics for a selected client.
 * Allows administrators to view any client's Google Analytics data.
 */

'use client';

import { GaMetricsGrid } from '@/components/analytics/GaMetricsGrid';
import { AdminGaAccountSelector } from '@/components/admin/admin-ga-account-selector';
...
interface AdminGaMetricsProps {
  clientId: string;
}

/**
 * @component AdminGaMetrics
 * Admin version of GA metrics component for viewing client analytics data.
 * 
 * Features:
 * - Client-specific GA account and property selection
 * - Same analytics display as client dashboard
 * - Admin-level access to any client's data
 * - Date range selection and filtering
 * 
 * @param {AdminGaMetricsProps} props - Component props
 */
export function AdminGaMetrics({ clientId }: AdminGaMetricsProps) {
  ...
  const fetchGaMetrics = useCallback(async (dateRange?: { from: Date; to: Date }) => {
    ...
    try {
      ...
      let url = '/api/admin/client-ga-metrics';
      ...
      params.append('clientId', clientId); // Add client ID for admin access
